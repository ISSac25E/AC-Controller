...
bool RelayVal = false; // << keep track of and change Relay Value As needed
...
/////////////// Interval Setup /////////////
////////////////////////////////////////////
uint32_t IntervalTimer = 0; // store millis value of start of interval
uint16_t Interval = 0;      // store interval length (min)
...
Par_uint16_t TY_Interval = 0;    // range 0 - (24 * 60). Current Interval Count in Minutes
...
void setup()
{
  ...
  //////// Pin Setup ////////
  {
    ...
    // RelayPin Setup:
    RelayOutput.digitalWrite(RelayVal);
    RelayOutput.pinMode(OUTPUT);
...
inline void checkUpdateHandle()
{
  ...
  if (millis() - dpid_update_timer >= DPID_UpdateInterval && DPID_UpdateInterval)
  {
    ...
    {
      tuya_module.mcu_dp_update(IntervalOn_DPID, TY_OnInterval, 1);
      dpid_update_timer = millis(); // << reset update timer
    }
    if (TY_OffInterval.change())
    {
      tuya_module.mcu_dp_update(IntervalOff_DPID, TY_OffInterval, 1);
      dpid_update_timer = millis(); // << reset update timer
    }

    if (TY_Interval.change())
    {
      tuya_module.mcu_dp_update(IntervalCount_DPID, TY_Interval, 1);
      dpid_update_timer = millis(); // << reset update timer
    }
  }
}
...
{
  if (Interval) // interval currently running
  {
    if (millis() - IntervalTimer >= ((uint32_t)Interval * (uint32_t)60 * (uint32_t)1000)) // Interval
    {
      RelayVal = !RelayVal; // at the end of an interval, always switch relay state
      Interval = 0;         // set to no Interval. Check if needed to reset later
      ...
      else
      {
        if (RelayVal)
        {
          TY_AcControl = 0; // switch state to "On"
        }
        else
        {
          TY_AcControl = 1; // switch state to "Off"
        }
      }
    }
  }
  ...
  if (TY_AcControl == 2)
  {
    ...
    if (!Interval)
    {
      // Interval should always be running in Auto Mode
      if (RelayVal)
      {
        Interval = TY_OnInterval;
        if (!Interval)
...
inline void statusHandle()
{
  uint16_t timer = ((millis() - IntervalTimer) / (uint32_t)60 / (uint32_t)1000);
  if (Interval >= timer)
    TY_Interval = Interval - timer;
  else
    TY_Interval = 0;
}
...
inline void EEPROM_handle()
{
  ...
  ac_mode = TY_AcControl;
  intervals[0] = TY_OnInterval;
  intervals[1] = TY_OffInterval;
...
unsigned char dp_process(unsigned char dpid, const unsigned char value[], unsigned short length)
{
  ...
  if (dpid == Control_DPID)
  {
    ...
    tuya_module.mcu_dp_update(dpid, TY_AcControl, 1);

    if (TY_AcControl == 0)
      RelayVal = true;
    if (TY_AcControl == 1)
      RelayVal = false;
    Interval = 0; // cancel any interval
  }
  else if (dpid == IntervalOn_DPID)
  {
    TY_OnInterval = tuya_module.mcu_get_dp_download_data(dpid, value, length);
    if (TY_OnInterval < 1)
      TY_OnInterval = 1;
    tuya_module.mcu_dp_update(dpid, TY_OnInterval, 1);
  }
  else if (dpid == IntervalOff_DPID)
  {
    TY_OffInterval = tuya_module.mcu_get_dp_download_data(dpid, value, length);
    if (TY_OffInterval < 1)
      TY_OffInterval = 1;
    tuya_module.mcu_dp_update(dpid, TY_OffInterval, 1);
  }
  else if (dpid == IntervalCount_DPID)
  {
    TY_Interval = tuya_module.mcu_get_dp_download_data(dpid, value, length);
    tuya_module.mcu_dp_update(dpid, TY_Interval, 1);

    IntervalTimer = millis();
    Interval = TY_Interval;
  }
  // return:
  return TY_SUCCESS;
}
